...
| **Prettier** | Formatting |
| **Vitest** | Unit tests (domain, application, infrastructure) |
| **Playwright** | Minimal smoke tests for renderer |
| **Husky + lint-staged** | Pre-commit guard |
| **GitHub / GitLab CI** | `pnpm build -r && pnpm lint -r && pnpm test -r` |
...
1. **Unit tests (Vitest)** – Run in every package; 80 % coverage gate.
...
5. **CI hint** – remind them to run `pnpm test -r && pnpm lint -r`.
